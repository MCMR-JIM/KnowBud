...
def test_mastery_requires_depth_application() -> None:
    ...
    assert state.mastery_map["t1"].mastery_state in {"practicing", "reinforced", "mastered"}
...
def test_mastery_state_can_decay_after_long_gap() -> None:
    state = LearningState(current_phase=LearningPhase.LEARNING, current_topic_id="t1")
    engine = MasteryEngine(spaced_gap_hours=1)

    engine.update_from_answer(state=state, topic_id="t1", user_text="因为能应用，所以答案是5", is_correct=True)
    node = state.mastery_map["t1"]
    node.last_success_ts = (datetime.now(timezone.utc) - timedelta(hours=5)).isoformat()
    update = engine.update_from_answer(state=state, topic_id="t1", user_text="", is_correct=False)

    assert update.topic_id == "t1"
    assert state.mastery_map["t1"].mastery_state == "decaying"

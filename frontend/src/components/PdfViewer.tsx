...
// 🌟 仅保留最基础的传值逻辑，UI 保持你原本最喜欢的样子
interface PdfViewerProps {
  url: string;
  onRenderComplete?: (currentPage: number, totalPages: number) => void;
}

// 在 onDocumentLoadSuccess 和 changePage 中：
...
export default function PdfViewer({ url, onRenderComplete }: PdfViewerProps) {
  ...
  function onDocumentLoadSuccess({ numPages }: { numPages: number }) {
    ...
    // 🌟 修改：把 numPages 传出去
    if (onRenderComplete) onRenderComplete(1, numPages);
  }
  const changePage = (offset: number) => {
    const newPage = pageNumber + offset;
    setPageNumber(newPage);
    // 🌟 修改：把 numPages 传出去
    if (onRenderComplete) onRenderComplete(newPage, numPages);
  };
      ...
      {numPages > 0 && (
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-30 flex items-center gap-4 bg-white/90 backdrop-blur-md px-8 py-4 rounded-full shadow-lg border border-purple-50 transition-all">
          {Array.from({ length: numPages }).map((_, i) => (
            <div
              key={i}
              className={`rounded-full transition-all duration-300 ${pageNumber === i + 1
